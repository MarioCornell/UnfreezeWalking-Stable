...
namespace UltimateReplay
{
    ...
    public sealed class ReplayObject : MonoBehaviour, IReplaySerialize, ISerializationCallbackReceiver
    {
...
#endif
            {
                if (IsReplaying == true)
                {
                    ...
                    for (int i = 0; i < runtimeComponents.Count; i++)
                        runtimeComponents[i].UpdateReplayVariables(playbackOperation.FrameDeltaT);
                }
            }
        }
        ...
        {
            // Don't allow in edit mode since prefabs will be detected automatically
            if (Application.isPlaying == false)
                throw new NotSupportedException("Cannot be called in edit mode");

            // Set prefab
            isPrefab = true;
            prefabType = ReplayPrefabType.PrefabAsset;

            // Update prefab id
            if (prefabIdentity.IsValid == false)
            ...
            for(int i = 0; i < events.Count; i++)
            {
                ...
                if(behaviour != null)
                {
                    ...
                    try
                    {
                        ...
                        // Don't pass null state to the user method
                        if (eventState == null)
        ...
        private void DeserializeReplayMethods(ReplayState state, IList<ReplayMethodData> methods) 
        {
            for(int i = 0; i < methods.Count; i++)
            {
                ...
                if(behaviour != null)
                {
                    ...
                    try
                    {
                        // Call the method on the behaviour
