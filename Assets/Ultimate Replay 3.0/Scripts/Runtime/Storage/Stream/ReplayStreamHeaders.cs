    ...
    public abstract partial class ReplayStreamStorage
    {
        protected internal class ReplayStreamHeader : IReplayStreamSerialize, IReplayTokenSerialize
        {
            ...
            void IReplayStreamSerialize.OnReplayStreamDeserialize(BinaryReader reader)
            {
                ...
                // Check for corrupted
                if (duration == 0f && snapshotCount == 0 && segmentTableOffset == 0 && persistentDataOffset == 0 && metadataOffset == 0)
                    throw new InvalidDataException("The specified replay stream appears to be corrupted! This can happen if the game quits without calling `StopRecording`");
            }
            #endregion
        }
