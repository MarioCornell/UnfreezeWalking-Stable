...
namespace UltimateReplay.Storage
{
    ...
    public sealed class ReplaySnapshot : IDisposable, IReplayReusable, IReplayStreamSerialize, IReplayTokenSerialize
    {
        ...
        {
            writer.Write(Common32.ToInteger(timeStamp));
            ...
            {
                // Write the identity
                ReplayIdentity id = objectState.Key;
                id.OnReplayStreamSerialize(writer);
        ...
        {
            timeStamp = Common32.ToSingle(reader.ReadInt32()); //reader.ReadSingle();
        ...
        public bool CopyTo(ReplaySnapshot destination, IReadOnlyList<ReplayIdentity> copyIdentities = null)
        {
            // Check for null
            if (destination == null)
            ...
            {
                // Check for copied
                if (copyIdentities != null && copyIdentities.Contains(state.Key) == false)
                    continue;
