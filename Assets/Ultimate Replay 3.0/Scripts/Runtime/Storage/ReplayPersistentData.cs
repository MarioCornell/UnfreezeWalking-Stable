...
namespace UltimateReplay.Storage
{
    public sealed class ReplayPersistentData : IReplayStreamSerialize, IReplayTokenSerialize
    {
        ...
        public ReplayState FetchPersistentDataByTimestamp(ReplayIdentity id, float timestamp)
        {
            ...
            {
                // Prepare for read
                list[index].persistentData.PrepareForRead();
        ...
        public bool CopyTo(ReplayPersistentData destination, IReadOnlyList<ReplayIdentity> copyIdentities = null)
        {
            // Check for null
            if (destination == null)
            ...
            if (persistentDataByTimestamp != null)
            {
                ...
                {
                    // Check for copied
                    if (copyIdentities != null && copyIdentities.Contains(dataByTimestamp.Key) == false)
                        continue;
            ...
            if (persistentData != null)
            {
                ...
                {
                    // Check for copied
                    if (copyIdentities != null && copyIdentities.Contains(data.Key) == false)
                        continue;
