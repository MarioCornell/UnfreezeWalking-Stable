...
namespace UltimateReplay.Storage
{
    ...
    public abstract class ReplayStorage : IDisposable
    {
        ...
        /// <param name="copyIdentities">An optional list of identities that should be copied. Can be used to only copy a subset of replay data, or leave null if all data should be copied</param>
        ...
        public bool CopyTo(ReplayStorage destination, IReadOnlyList<ReplayIdentity> copyIdentities = null)
        {
            // Check for disposed
            CheckDisposed();
            ...
            do
            {
                ...
                if(current != null)
                {
                    ...
                    // Perform copy
                    if(current.CopyTo(clone, copyIdentities) == false)
                    {
                        Debug.LogError("Failed to copy snapshot: " + sequenceId);
                        return false;
                    }
            ...
            if (persistentData.CopyTo(destination.persistentData, copyIdentities) == false)
            {
        ...
        /// <exception cref="ArgumentNullException">Destination storage is null</exception>
        /// <exception cref="ObjectDisposedException">This <see cref="ReplayStorage"/> or destination <see cref="ReplayStorage"/> is disposed</exception>
        public ReplayAsyncOperation CopyToAsync(ReplayStorage destination, IReadOnlyList<ReplayIdentity> copyIdentities = null)
        {
            // Check for null
            if (destination == null)
                throw new ArgumentNullException(nameof(destination));
            ...
            ThreadPool.QueueUserWorkItem((object state) =>
            {
                ...
                do
                {
                    ...
                    if (current != null)
                    {
                        ...
                        if(current.CopyTo(clone, copyIdentities) == false)
                        {
                            async.Complete(false, "Failed to copy snapshot: " +  sequenceId);
                            return;
                        }
