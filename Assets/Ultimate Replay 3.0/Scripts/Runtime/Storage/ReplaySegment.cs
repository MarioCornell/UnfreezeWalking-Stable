...
namespace UltimateReplay.Storage
{
    public sealed class ReplaySegment : IDisposable, IReplayReusable, IReplayStreamSerialize, IReplayTokenSerialize
    {
        ...
        private static readonly ThreadLocal<List<ReplayIdentity>> sharedIdentities = new ThreadLocal<List<ReplayIdentity>>(() => new List<ReplayIdentity>(64));           // Must be unique instance per thread
        private static readonly ThreadLocal<List<ReplaySnapshot>> sharedSnapshots = new ThreadLocal<List<ReplaySnapshot>>(() => new List<ReplaySnapshot>(64));            // Must be unique instance per thread
        ...
        public void CompressSegment()
        {
            ...
            // Get working collections
            List<ReplayIdentity> threadIdentities = sharedIdentities.Value;
            List<ReplaySnapshot> threadSnapshots = sharedSnapshots.Value;

            // Add to list for quicker indexing and sorting
            threadSnapshots.AddRange(snapshots.Values);

            // Process all snapshots in reverse
            // Stop at 1 because the first snapshot cannot be compressed
            for(int i = threadSnapshots.Count - 1; i >= 1; i--)
            {
                // Get all identities in a separate iterator so that we can modify replay state without modifying collection
                threadIdentities.AddRange(threadSnapshots[i].Identities);

                // Process all identities
                for(int j = 0; j < threadIdentities.Count; j++)
                {
                    ReplayIdentity identity = threadIdentities[j];

                    // Get current state
                    IReplaySnapshotStorable currentState = threadSnapshots[i].GetReplayObjectState(identity);
                    ...
                    if(currentState.StorageType == ReplaySnapshotStorableType.StateStorage)
                    {
                        ...
                        for(int k = 0; k < i; k++)
                        {
                            ...
                            ReplayState dataState = threadSnapshots[k].GetReplayObjectState(identity) as ReplayState;
                    ...
                    if (overrideState != currentState)
                        threadSnapshots[i].OverrideStateDataForReplayObject(identity, overrideState);
                }
        ...
        public void DecompressSegment()
        {
            ...
            for (int i = 1; i < threadSnapshots.Count; i++)
            {
                // Get all identities in a separate iterator so that we can modify replay state without modifying collection
                threadIdentities.AddRange(threadSnapshots[i].Identities);

                // Process all identities
                foreach (ReplayIdentity identity in threadIdentities)
                {
                    // Get current state
                    IReplaySnapshotStorable currentState = threadSnapshots[i].GetReplayObjectState(identity);
                    ...
                    if (currentState.StorageType == ReplaySnapshotStorableType.StatePointer)
                    {
                        ...
                        ReplayState dataState = threadSnapshots[((ReplayStatePointer)currentState).snapshotOffset].GetReplayObjectState(identity) as ReplayState;
                        ...
                        // Override the data
                        threadSnapshots[i].OverrideStateDataForReplayObject(identity, dataState);
                    }
                }
        ...
        internal void CorrectSequenceId(ref int currentSequenceId)
        {
            ...
            foreach (ReplaySnapshot snapshot in threadSnapshots)
            {
