...
namespace UltimateReplay
{
    public sealed class ReplayRiggedHumanoid : ReplayRecordableBehaviour
    {
        ...
        private Vector3 bodyReferencePosition = Vector3.zero;
        private Quaternion bodyReferenceRotation = Quaternion.identity;
        ...
        [SerializeField]
        [HideInInspector]
        internal RecordPrecision muscleValuePrecision = RecordPrecision.HalfPrecision16Bit;
        ...
        public RecordPrecision MuscleValuesPrecision
        {
        ...
        private void Start()
        {
            ...
            bodyReferencePosition = currentPose.bodyPosition - transform.position;
            bodyReferenceRotation = currentPose.bodyRotation;
        }
        ...
        protected override void OnReplayUpdate(float t)
        {
            ...
            if ((replayBodyPosition & RecordFullAxisFlags.Interpolate) != 0 && (replayBodyPosition & RecordFullAxisFlags.XYZ) != 0)
            {
                ...
                currentPose.bodyPosition = transform.InverseTransformPoint(Vector3.Lerp(lerpPoseFrom.bodyPosition, lerpPoseTo.bodyPosition, t));
                updatePose = true;
            }
            ...
            {
                currentPose.bodyRotation = Quaternion.Inverse(bodyReferenceRotation) * Quaternion.Lerp(lerpPoseFrom.bodyRotation, lerpPoseTo.bodyRotation, t);
                updatePose = true;
            }
        ...
        public override void OnReplayDeserialize(ReplayState state)
        {
            ...
            // update body rotation if interpolation is dabbled
            ...
            // Update muscle values if interpolation is disabled
        ...
        private void UpdatePoseHandler()
        {
            ...
            if (poseHandler == null && observedAnimator != null && observedRoot != null && observedAnimator.avatar != null)
            {
                ...
                poseHandler.GetHumanPose(ref currentPose);
                poseHandler.GetHumanPose(ref lerpPoseFrom);
                poseHandler.GetHumanPose(ref lerpPoseTo);
            }
        }
